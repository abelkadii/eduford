...
function ColorMix1(dyePercentage, color1, color2) {
  ...
  var mixRatio = dyePercentage / 100;

  // Extract RGB values from each color
  var red1 = parseInt(color1.substring(1, 3), 16);
  var green1 = parseInt(color1.substring(3, 5), 16);
  var blue1 = parseInt(color1.substring(5, 7), 16);

  var red2 = parseInt(color2.substring(1, 3), 16);
  var green2 = parseInt(color2.substring(3, 5), 16);
  var blue2 = parseInt(color2.substring(5, 7), 16);

  // Interpolate RGB values based on the mix ratio
  var mixedRed = Math.round(red1 * (1 - mixRatio) + red2 * mixRatio);
  var mixedGreen = Math.round(green1 * (1 - mixRatio) + green2 * mixRatio);
  var mixedBlue = Math.round(blue1 * (1 - mixRatio) + blue2 * mixRatio);
      ...
      if (age != "") {
        if (gender) {
          if (values.length > 2) {
            ...
            var cigar = document.querySelector("input[name=cigar]:checked")?.value;
            var preg = document.querySelector("input[name=pregyesno]:checked")?.value;
            // console.log(preg);
            var trisemister = document.querySelector("input[name=trisemister]:checked")?.value;
            document.getElementById('predictbtn').innerHTML='Diagnosing'
            $.ajax({
              url: "/predictor/send_data/",
              headers: {'X-CSRFToken': csrftoken},
              type: "POST",
              ...
              document.getElementById('backbtn').disabled = true
                ...
                if (response.length > 0) {
                  ...
                  for (var i = 0; i < response.length; i++) {
                    ...
                    var resultElement1 = document.createElement("div");
                    resultElement1.style.width = `${Math.floor(probability)}%`
                    var mixedColor = ColorMix1(probability, '#5be85b', '#f63e3e');
...
function onButtonClick(disease, probability) {
  ...
  // console.log(csrftoken)
  $.ajax({
    url: "/predictor/medic/", // Update this URL to match your API route
    headers: {'X-CSRFToken': csrftoken},
    type: "GET",
    success: function (response) {
...
window.onload = function () {
  document.getElementById("print").addEventListener("click", e=>{
    document.getElementById("print").innerHTML="prepearing checkout ..."
    fetch(`/predictor/get-report/${id}`)
    .then(res=>res.json())
    .then(res=>{
      if(res.success==true){
        window.location.href = res.redirect
      }else{
        document.getElementById("print").innerHTML="Get Report"
        alert(res.message)
      }
    }).catch(r=>{
      document.getElementById("print").innerHTML="Get Report"
